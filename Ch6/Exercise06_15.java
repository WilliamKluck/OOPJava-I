...
public class Exercise06_15 {
	...
	public static void main(String[] args) {
		...
		System.out.printf("%-14s | %-20s | %-20s | %-20s | %-20s%n", "Taxable", "Single", "Married Joint or", "Married", "Head of" );
		System.out.printf("%-38s %-22s %-22s %s%n", "Income", "Qualifying Widow(er)", "Seperate", "a House");
		for (int income = 50000; income <= 60000; income += 50) {
			System.out.printf("%-16d", income);
			for(int status = 0; status <= 3; status ++) {
				System.out.printf("%-23d", Math.round(computeTaxes(status, income)));
			}
			System.out.println();
		}
	}
	public static double computeTaxes(int status, double income) {
		double tax = 0;
